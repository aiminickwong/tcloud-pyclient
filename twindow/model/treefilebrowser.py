...
from twindow.common.exception import MountException
import os
...
LOG = logging.getLogger("File.Plugin")

class TreeFileBrowser(TcloudObject):
    ...
    def check_path(self,path):
        if ospath.exists(path) and os.listdir(path) != []:
            self.root = path
        else:
            host = self.conf['host']
            _dir = self.conf['dir']
            user = self.conf["user"]
            pwd = self.conf["pwd"]
            _,error = utils.smb_mount(host, _dir, user, pwd)
            if error:
                LOG.error("error in mounting file system" % error)
    ...
    def set_active_dir(self, directory):
        ...
        if  directory == rootdir:
            ...
            for i in range(depth):
                ...
                if _iter == None: continue
                val = model.get_value(_iter, 1).replace('/','') + '/'
                while val != newpath:
    ...
    def create_popup(self):
        ...
        self.hidden_check_menu = gtk.CheckMenuItem("Show hidden files")
