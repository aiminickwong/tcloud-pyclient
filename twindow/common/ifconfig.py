...
class Interface(object):
    ...
    def set_network(self,dev,ip,netmask,gateway):
        utils.execute('cp','/etc/network/interfaces','/etc/network/interfaces.bak')
        network_template = '''#generate by tcloud
auto lo
iface lo inet loopback

auto %(ifname)s
iface %(ifname)s inet static
address %(address)s
netmask %(netmask)s
gateway %(gateway)s''' % ({'ifname':dev,
                'address':ip,
                'netmask':netmask,
                'gateway':gateway})
        ...
        utils.execute('/etc/init.d/networking','restart')
